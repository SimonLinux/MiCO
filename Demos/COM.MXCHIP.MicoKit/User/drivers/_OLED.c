...
#define oled_log_trace() custom_log_trace("OLED")
...
//	return SPI_I2S_ReceiveData(OLED_SPIx); 
  ...
  oled_spi_msg.tx_buffer = &tx_data;
...
//	OLED_RES_RESET();
   ...
   MicoGpioInitialize( (mico_gpio_t)OLED_CS_PIN, OUTPUT_PUSH_PULL );
   OLED_CS_SET();
   MicoGpioInitialize( (mico_gpio_t)OLED_DC_PIN, OUTPUT_PUSH_PULL );
   OLED_DC_RESET();
//   MicoGpioInitialize( (mico_gpio_t)OLED_RES_PIN, OUTPUT_PUSH_PULL );
//   OLED_RES_RESET();
}
...
//	SPI_Cmd(OLED_SPIx, ENABLE); 
  ...
  require_noerr_action( err, exit, oled_log("ERROR: MicoSpiInitialize (micokit_spi_oled) err=%d.", err) );
