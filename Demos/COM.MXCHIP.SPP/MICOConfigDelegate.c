...
USED void m_system_delegate_config_will_start( void )
{
...
USED void m_system_delegate_config_will_stop( void )
{
  config_delegate_log_trace();
...
USED void m_system_delegate_config_recv_ssid ( void )
{
  config_delegate_log_trace();
...
USED void m_system_delegate_config_success( mico_config_source_t source )
{
  config_delegate_log( "Configed by %d", source );
}


USED OSStatus m_system_delegate_config_recv_auth_data(char * anthData  )
{
  config_delegate_log_trace();
  (void)(anthData);
  return kNoErr;
...
json_object* ConfigCreateReportJsonMessage( mico_Context_t * const inContext )
{
  ...
  application_config_t *appConfig = m_system_context_get_user_data( inContext );
